  ...
  return (
    <header className="bg-primary py-2 flex px-6 justify-between">
      ...
      <nav className="flex items-center">
        ...
        <div className={`flex flex-col content-around flex-1 sm:hidden bg-primary bg-opacity-90 rounded-bl-xl absolute w-4/5 top-10  py-20 duration-500 z-50 ${open ? "hidden" : "right-0"
          }`}>
          {navigation.map((link) => (
            <Link key={link.name}

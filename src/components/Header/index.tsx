    ...
    <header className="bg-primary py-2 flex px-6 justify-between">
      ...
      <nav className="flex items-center">
        ...
        <div className={`flex flex-col content-around flex-1 sm:hidden bg-primary bg-opacity-90 rounded-bl-xl fixed w-4/5 top-10  py-20 duration-500 z-50 ${open ? "right-[-100%]" : "right-0"
          ...
          {navigation.map((link) => (
            <a key={link.name}
              className={`mx-10 my-2 px-3 py-1 cursor-pointer text-white text-[10px] font-extrabold hover:bg-black hover:bg-opacity-30 rounded-md transition-colors ${path == link.redirect ? "bg-black bg-opacity-30" : ""}`}>{link.name}</a>
          ))}
        </div>
        ...
        <div className="hidden sm:block">
          {navigation.map((link) => (
            <a key={link.name}
              className={`mx-10 px-3 py-1 cursor-pointer text-white text-[10px] font-extrabold hover:bg-black hover:bg-opacity-30 rounded-md transition-colors ${path == link.redirect ? "bg-black bg-opacity-30" : ""}`}>{link.name}</a>
          ))}
        </div>
      </nav>
    </header>
